...
for k = 1:n_dset
    ...
    % 🔧 전류가 실제로 방전 중인 부분만 사용 (C-rate < 0)
    idx_dis_cd = idx_dis_cd( C_raw(idx_dis_cd) < -1e-3 );   % 임계값은 필요하면 조정
    ...
    % 🔧 같은 Capacity에서 T만 다른 점들(세로 꼬리) 제거
    [Q_cd, ia] = unique(Q_cd, 'stable');  % 처음 나오는 점만 유지
    Tm_cd = Tm_cd(ia);
    Ta_cd = Ta_cd(ia);
    ...
    [Q_max_cd, iQmax_cd] = max(Q_plot_cd);

    Tm_at_Qmax = Tm_cd(iQmax_cd);
    Ta_at_Qmax = Ta_cd(iQmax_cd);

    % Tmax 세로선
    plot(ax3, [Q_max_cd Q_max_cd], [25 Tm_at_Qmax], '-', ...
        'Color', col, 'LineWidth', lw);

    % Tavg 세로선
    plot(ax4, [Q_max_cd Q_max_cd], [25 Ta_at_Qmax], '-', ...
        'Color', col, 'LineWidth', lw);
